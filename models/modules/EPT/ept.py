...
class XTransEncoderAct(nn.Module):
    ...
    def feature_map(self, H, Z, block_id, batch_id, edges, edge_attr=None):
        return self.encoder.returnf(H, Z, block_id, batch_id, edges, edge_attr)
...
class Transformer(nn.Module):
    ...
    def returnf(self, H, Z, block_id, batch_id, edges, edge_attr=None):
        with torch.no_grad():
            (unit_row, unit_col), (block_edge_id, unit_edge_src_start, unit_edge_src_id) = _unit_edges_from_block_edges(block_id, edges.T, Z, k=self.sparse_k) # [Eu], Eu = \sum_{i, j \in E} n_i * n_j
        ...
        for i in range(self.n_layers):
            ...
            H_lst.append(H_batch[H_mask])
            V_lst.append(V_batch[H_mask])
