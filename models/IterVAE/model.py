...
class CondIterAutoEncoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.encoder_struc = create_net(encoder_type, hidden_size, edge_size, encoder_struc_opt)
        self.encoder_block = create_net(encoder_type, hidden_size, edge_size, encoder_block_opt)
        self.decoder_struc = create_net(decoder_type, hidden_size, edge_size, decoder_struc_opt)
        self.decoder_block = create_net(decoder_type, hidden_size, edge_size, decoder_block_opt)

        self.is_aa_embedding = nn.Embedding(2, embed_size)
        ...
        self.dec_input_linear = nn.Linear(hidden_size * 3 + hidden_size, hidden_size) # atom, time, position, latent
        ...
        self.bb_mat = torch.tensor(VOCAB.freq_mat, dtype=torch.float)
        self.bb_temperature = 0.7
        self.bb_projection = MLP(
            input_size=self.bb_mat.shape[0],
            hidden_size=hidden_size,
        ...
        self.BB_FREQUENCY = torch.log(self.bb_mat + 1e-6)
    ...
    def forward(
        ...
        Zh, Zx, Zx_mu, signed_Zx_log_var, Zh_kl_loss, Zx_kl_loss, bind_site_repr, ligand_repr = self.encode(
            X, S, A, bonds, chain_ids, generate_mask, block_lengths, lengths, is_aa, binding_site_gen_mask=binding_site_gen_mask
        ) # [Nblock, d_latent], [Nblock, 3], [1], [1]


        pos_logits = self.dti_mlp(torch.cat([bind_site_repr, ligand_repr], dim=-1))
        neg_logits = self.dti_mlp(torch.cat([bind_site_repr, ligand_repr.roll(-1, dims=0)], dim=-1))
        dti_logits = nn.Sigmoid()(torch.cat([pos_logits, neg_logits], dim=0)).squeeze()
        dti_labels = torch.cat([torch.ones(batch_size), torch.zeros(batch_size)], dim=0).to(pos_logits.device).float()
        ...
        Zh = self.dec_latent2hidden(Zh)
        Zh, Zx = self.decoder_block(Zh, Zx, latent_block_ids, batch_ids, edges, edge_attr)
        ...
        frontier_to_pred, frontier_label = torch.flatten(bond_to_pred), (bond_label>0).repeat(2).long()
        pred_frontier_logits = self.frontier_mlp(H_t[frontier_to_pred])
        frontier_loss = F.cross_entropy(input=pred_frontier_logits, target=frontier_label)
        if torch.isnan(frontier_loss): frontier_loss = 0.0
        ...
        loss_dict = {
            ...
            'dti_loss': F.mse_loss(dti_logits, dti_labels, reduction='none').mean(),
            'frontier_loss': frontier_loss,
            'center_loss': torch.sqrt(scatter_mean(((block_centers - Zx) ** 2).sum(-1), batch_ids, dim=0)).mean(),
        ...
        with torch.no_grad():
            ...
            # record dti prediction
            loss_dict.update({
                'dti_accu': ((dti_logits>0.5)==dti_labels).float().mean(),
            })
            loss_dict.update({
                'frontier_accu': (torch.argmax(pred_frontier_logits, dim=-1) == frontier_label).long().sum() / (len(frontier_label) + 1e-10)
            })
        return loss_dict
    
    def encode(self, X, S, A, bonds, chain_ids, generate_mask, block_lengths, lengths, is_aa, deterministic=False, binding_site_gen_mask=None):

        batch_ids = length_to_batch_id(lengths)
        block_ids = length_to_batch_id(block_lengths)

        self.BB_FREQUENCY = self.BB_FREQUENCY.to(S.device)
        
        H_prior = self.BB_FREQUENCY[S] # [Nblock, T]
        H_add = self.bb_projection(torch.softmax(H_prior / self.bb_temperature, dim=-1))

        # H = self.embedding(S, A, block_ids) #+ self.rel_pos_embedding(position_ids)[block_ids] # [Natom, embed_size]
        H = self.embedding(S, A, block_ids) + self.ctx_embedding(generate_mask[block_ids].long()) + self.is_aa_embedding(is_aa.long())[block_ids] + H_add[block_ids]
        ...
        attn_mask_block = self.get_attn_mask(batch_ids, torch.arange(len(batch_ids)), generate_mask)

        # make bonds bidirectional
        bond_row, bond_col, bond_type = bonds.T
        Zh_atom, Zx_atom = self.encoder_struc(
        ...
        # encode more
        latent_block_ids = torch.ones_like(batch_ids).cumsum(dim=-1) - 1
        edges, edge_type = self.get_edges(batch_ids, chain_ids, scatter_mean(X, block_ids, dim=0), latent_block_ids, generate_mask, False, False)
        edge_attr = self.edge_embedding(edge_type)
        Zh, Zx = self.encoder_block(
            Zh, scatter_mean(X, block_ids, dim=0), latent_block_ids, batch_ids, edges=edges, edge_attr=edge_attr,
    ...
    def decode_block_type(self, Zh, Zx, chain_ids, lengths):
        ...
        edges, edge_type = self.get_edges(batch_ids, chain_ids, Zx, latent_block_ids, None, True, True)
        edge_attr = self.edge_embedding(edge_type)
        block_h, _ = self.decoder_struc(Zh, Zx, latent_block_ids, batch_ids, edges, edge_attr)
    ...
    def decode_structure(self, Zh, X, A, position_ids, topo_edges, topo_edge_attr, chain_ids, batch_ids, block_ids, t):
        ...
        H_t, X_next = self.decoder_struc(H, X, block_ids, batch_ids, edges, edge_attr, topo_edges, topo_edge_attr) # [Natom', hidden_size], [Natom', 3]
        ...
        **kwargs
        ...
        if 'given_latent' in kwargs:
            ...
            Zh, Zx, _, _, _, _, _, _ = self.encode(
                X, S, A, bonds, chain_ids, generate_mask, block_lengths, lengths, is_aa, deterministic=True
            ) # [Nblock, d_latent], [Nblock, 3], [1], [1]
        block_ids = length_to_batch_id(block_lengths)
        batch_ids = length_to_batch_id(lengths)
    ...
    def extract_decoder_feature(self, Zh, Zx, chain_ids, lengths):
        '''
            Args:
        ...
        Zh = self.dec_latent2hidden(Zh)
        Zh, Zx = self.decoder_block(Zh, Zx, latent_block_ids, batch_ids, edges, edge_attr)
        H_map, Z_map = self.decoder_struc.feature_map(Zh, Zx, latent_block_ids, batch_ids, edges, edge_attr)
        return H_map, Z_map
    
    def generate_block_type(
        self,
        Zh,
        Zx, 
        chain_ids,          # [Nblock], split different chains
