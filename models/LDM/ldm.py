...
#from .flowmatching import FullFlow
...
class LDMMolDesign(nn.Module):
    ...
    def __init__(
        ...
        self.lpl_w = diffusion_opt['lpl_loss']
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            Zh, Zx, _, _, _, _, _, _ = self.autoencoder.encode(
                X, S, A, bonds, chain_ids, generate_mask, block_lengths, lengths, is_aa, deterministic=self.latent_deterministic
        ...
        loss_dict = self.diffusion.forward(
            ...
            lengths=lengths,
            vae_model=self.autoencoder
        )

        # loss
        # LPL loss only for fine-tuning
        if self.lpl_w:
            loss_dict['total'] = loss_dict['H'] * self.h_loss_weight + loss_dict['X'] + loss_dict['X_feat'] * self.lpl_w + loss_dict['H_feat'] * self.lpl_w
        else:
