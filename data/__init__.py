...
from .moleonly import MoleonlyDataset
...
def create_dataloader(dataset, config: dict, n_gpu: int=1, rank: int=-1):
    ...
    if n_gpu > 1:
        sampler = torch.utils.data.distributed.DistributedSampler(dataset, num_replicas=n_gpu, rank=rank,shuffle=shuffle)
